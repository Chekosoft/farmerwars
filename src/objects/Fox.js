...
export default class Fox extends Phaser.Sprite {
	constructor(game) {
		...
		super(game, x*config.width, y*config.height, (y == 0) ? 'fox_down' : 'fox_up');
		this.orientation = (y == 0) ? 'down' : 'up';
		this.grid_x = x;
		this.grid_y = y;
		this.isSleeping = false;
    this.canMove = true;
		this.nearestChicken = Fox.getNearestChicken(game, x, y);
	...
	moveToChicken() {
		let rel_x = this.grid_x - this.nearestChicken.x;
		let rel_y = this.grid_y - this.nearestChicken.y
      ...
      this.current_floor = state._getFloorAt(this.grid_x, this.grid_y);
			...
			if(this.current_floor.groundType == 'grains' && this.current_floor.animals > 0){
				this.eatChicken();
			} else if((this.current_floor.groundType != 'grains'
  ...
  resumeMovementLoop() {
	...
	isTargetStillChicken() {
      ...
      this.current_floor.groundType == 'grains' &&
      this.current_floor.animals > 0) {
			return;
		} else {
			this.nearestChicken = Fox.getNearestChicken(this.game, this.grid_x, this.grid_y);
		}
	}
	...
	eatChicken() {
		console.log('Holi');
		if(this.current_floor.groundType == 'grains' && this.current_floor.animals > 0) {
			this.current_floor.hasFox = true;
      this.current_floor.setFox(this);
  ...
  isFoxSleepy() {
	...
	goToSleep() {
		...
		this.loadTexture('fox_sleep');
		this.sleepEvent.add(Phaser.Timer.SECOND * 5, function() {
			this.eatenAnimals = 0;
			this.isSleeping = false;
			this.loadTexture('fox_' + this.orientation);
			this.isTargetStillChicken();
			this.resumeMovementLoop();
		}, this);
  ...
  stopAllTimers() {
	...
	static getNearestChicken(game, x, y) {
		var position = {x: -1, y: -1};
		let state = game.state.getCurrentState();
		let current_distance = -1;
		let found = false;

		for(let i = 0, max = state.chicken_lands.length; i < max; i++) {
			let field = state.chicken_lands[i];
		...
		if(found)
			...
			return state.chicken_farmer.getPosition();
		}
	}
}
